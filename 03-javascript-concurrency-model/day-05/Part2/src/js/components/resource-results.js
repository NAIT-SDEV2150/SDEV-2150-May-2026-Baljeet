...
class ResourceResults extends HTMLElement {
  ...
  // TODO: Stage 2: Observe the `source` attribute// without it you can't use attributechangedCallBack
  ...
  async #fetchData(source) {
    try {
      const respons = await fetch(source);
      if (!respons.ok) {
        throw new Error(`Network response was not ok: ${respons.statusText}`);
      }
  ...
  // - set results with fetched data
   ...
   attributeChangedCallback(name, oldValue, newValue) {
    if (name === 'source' && oldValue !== newValue) {
      // Check if connected before fetchingnpm 
      if (this.isConnected) {
